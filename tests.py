from tarkir.gurps.skill import (
    check_result,
...
if __name__ == '__main__':
    assert check_result(3, 5) == RESULT_CRITICAL_SUCCESS
    assert check_result(5, 5) == RESULT_SUCCESS
    assert check_result(6, 5) == RESULT_FAIL
    assert check_result(14, 5) == RESULT_FAIL
    assert check_result(16, 5) == RESULT_CRITICAL_FAIL
    assert check_result(18, 5) == RESULT_CRITICAL_FAIL

    assert check_result(3, 16) == RESULT_CRITICAL_SUCCESS
    ...
    assert check_result(18, 16) == RESULT_CRITICAL_FAIL

    assert check_result(3, 3) == RESULT_CRITICAL_SUCCESS
    ...
    assert check_result(3, 20) == RESULT_CRITICAL_SUCCESS
